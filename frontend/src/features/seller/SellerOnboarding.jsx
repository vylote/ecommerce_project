...
// import api from '../../shared/utils/api'; // Mở comment khi nối API
...
export default function SellerOnboarding() {
  ...
  const [shopData, setShopData] = useState({
    ...
    email: '',
    category: 'Thời trang',
    address: null, // Sẽ chứa object địa chỉ khi user thêm
  });
  ...
  const [tempAddress, setTempAddress] = useState({
    fullName: '', phone: '', province: '', detail: ''
  });
  ...
  const handleRegisterShop = async () => {
    ...
    try {
      // Giả lập gọi API đăng ký Shop
      // const payload = { ...shopData, shipping: shippingData };
      // await api.post('/shops/register', payload);
      
      await new Promise(resolve => setTimeout(resolve, 1500)); // Fake delay
      toast.success('Đăng ký Shop thành công! Chào mừng bạn đến với Kênh Người Bán.');
      navigate('/seller/dashboard'); // Chuyển vào trang quản trị shop
    } catch (error) {
          ...
          {currentStep === 1 && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="form-control">
                  <label className="label font-medium text-gray-700">Ngành hàng chính</label>
                  <select 
                ...
                <div className="form-control">
                  ...
                  <input 
                    type="email" className="input input-bordered focus:border-[#ee4d2d]" placeholder="email@domain.com"
              ...
              <div className="mt-8 border-t pt-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {!shopData.address && (
                    <button onClick={() => setShowAddressModal(true)} className="btn btn-sm btn-outline text-[#ee4d2d] hover:bg-[#ee4d2d] hover:text-white hover:border-[#ee4d2d]">
                      + Thêm địa chỉ
                    </button>
                  )}
                </div>
                ...
                {shopData.address ? (
                  <div className="p-4 border rounded-md bg-gray-50 flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-gray-600 text-sm mt-1">{shopData.address.detail}, {shopData.address.province}</div>
                    </div>
                    <button onClick={() => setShowAddressModal(true)} className="text-blue-600 text-sm hover:underline">Sửa</button>
                  </div>
                ) : (
            ...
            <div className="text-center animate-fade-in py-8">
              ...
              <div className="bg-gray-50 p-6 rounded-lg text-left max-w-md mx-auto border mb-8">
                <div className="grid grid-cols-3 gap-y-3 text-sm">
                  ...
                  <div className="text-gray-500">Ngành hàng:</div><div className="col-span-2 font-medium">{shopData.category}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-lg shadow-xl overflow-hidden animate-fade-in-up">
            <div className="px-6 py-4 border-b">
              <h3 className="font-bold text-lg">Địa chỉ Shop</h3>
            </div>
            ...
            <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3 border-t">
              ...
              <button 
                onClick={() => {
                  if(!tempAddress.fullName || !tempAddress.phone || !tempAddress.province) {
                    toast.error('Vui lòng điền đủ thông tin địa chỉ'); return;
                  }
                  setShopData({...shopData, address: tempAddress});
                  setShowAddressModal(false);
                }} 
                className="btn border-none bg-[#ee4d2d] hover:bg-[#d73211] text-white"
              >
                Lưu địa chỉ
              </button>
            </div>
